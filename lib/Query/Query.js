...
var Condition_2 = require("../Query/Condition");
var Sorter_2 = require("./Sorter");
var Query = (function () {
    ...
    Query.prototype.addWhere = function (type, conditions, bind) {
        var resolvedCondition = this._resolveTokens(conditions.trim(), bind);
        this.condition(Condition_1.default.parse(type, resolvedCondition));
        return this;
    };
    Query.prototype.andWhere = function (conditions, bind) {
        return this.addWhere(Condition_2.ConditionType.AND, conditions, bind);
    };
    Query.prototype.orWhere = function (conditions, bind) {
        return this.addWhere(Condition_2.ConditionType.OR, conditions, bind);
    };
    Query.prototype.where = function (conditions, bind) {
        return this.andWhere(conditions, bind);
    };
    Query.prototype.having = function (values) {
        var _this = this;
        _.each(values, function (value, key) {
            _this.condition(new Condition_1.default(Condition_2.ConditionType.AND, key, Condition_2.ConditionOperator.IS_EQUAL, value));
        });
        return this;
    };
    ...
    Query.prototype.orderBy = function (field, direction) {
        if (direction === void 0) { direction = Sorter_2.SortDirections.ASCENDING; }
        var resolvedDirection;
        if (_.isString(direction)) {
            resolvedDirection = direction == 'DESC' ? Sorter_2.SortDirections.DESCENDING : Sorter_2.SortDirections.ASCENDING;
        }
        else {
            resolvedDirection = direction;
        }
        return this.sorter(new Sorter_1.default(field, resolvedDirection));
    };
    ...
    Query.prototype._resolveTokens = function (input, tokens) {
        var _this = this;
        if (_.isObject(tokens)) {
            return input.replace(/:([^:]+):/g, function (token) {
                ...
                return _this._processToken(tokenValue);
            });
        }
        else {
    ...
    Query.prototype._processToken = function (token) {
