...
var ActiveModel = (function (_super) {
    ...
    ActiveModel.prototype.hasSavedData = function () {
        return !!this._savedData;
    };
    ActiveModel.prototype.makeSnapshot = function () {
        this.setSavedData(this.toObject());
    };
    ActiveModel.prototype.getChanges = function (recursive) {
        var _this = this;
        ...
        _.each(newData, function (val, key) {
            var oldVal = _this._savedData[key];
            var equal = _.isObject(oldVal) && _.isObject(val) ? _.isEqual(oldVal, val) : oldVal == val;
            if (!equal) {
                changes[key] = recursive ? Model_1.default.recursiveToObject(val, [_this]) : val;
            }
        });
        return changes;
    };
    ...
    ActiveModel.prototype.update = function (data) {
        ...
        return this._dataService.updateModel(this._resourceName, this, data).then(function () {
            return _this;
        });
    };
    ActiveModel.prototype.create = function (dataService, resourceName, data) {
        var _this = this;
        return dataService.createModel(resourceName, this, data).then(function () {
            return _this;
        });
    };
    ...
    ActiveModel.prototype.invalidate = function () {
        if (!this.isActivated()) {
            throw new Exception_1.default('Unable to invalidate ' + this.getResourceIdentifier() + ', model is not alive');
        }
